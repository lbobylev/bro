...
async function getHistory(days = 7): Promise<HistoryEntry[]> {
  ...
  try {
    ...
    console.log(`Read ${fileBuffer.byteLength} bytes.`);

    // Load the database
    db = new SQL.Database(fileBuffer);
    console.log("Database loaded into sql.js.");
    ...
    if (!results || results.length === 0) {
      console.log("History query returned no results.");
    ...
    const historyEntries: HistoryEntry[] = rows.map((row) => {
      ...
      return {
        id: `${url}-${visitTime}`, // Create a unique ID
        title: title || url,
        url: url,
    ...
    console.error("Error processing Brave history with sql.js:", error);
    if (
      ...
      showToast({
        ...
        message: "Brave history file might be corrupted or unreadable.",
      });
    } else if (error.code === "ERR_FS_FILE_TOO_LARGE") {
      showToast({
        style: Toast.Style.Failure,
        title: "History File Too Large",
        message: "Brave history file is too large to load into memory.",
      });
    } else {
      showToast({
        style: Toast.Style.Failure,
    ...
    if (db) {
      db.close();
      console.log("sql.js database closed.");
    }
  }
}
